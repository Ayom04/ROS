...
class DoTaskActionClient(Node):
    ...
    def feedback_callback(self, feedback_msg):
        # Accessing the 'progress' field we defined in the server earlier
    ...
    def get_result_callback(self, future):
        ...
        # Signal the node to stop spinning safely
        self.get_logger().info("Task Completed. Shutting down...")
        # Avoid calling rclpy.shutdown() directly inside a callback if possible

def main(args=None):
    rclpy.init(args=args)
    client = DoTaskActionClient()
    
    client.send_goal(10) # Using 10 seconds as a test
